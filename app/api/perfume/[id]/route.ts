...
export async function PUT(request: Request, { params }: { params: { id: string } }) {
  ...
  const { error: note_delete_error } = await createRouteHandlerClient({ cookies })
...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
  ...
  const { error } = await createRouteHandlerClient({ cookies }).from('perfume_list').delete().eq('p_id', +params.id);
  const { data, error: image_delete_error } = await createRouteHandlerClient({ cookies })
